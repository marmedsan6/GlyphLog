...
Esta rama usa un subconjunto portable de Gentle AI v2.5.0-rc.1:

- runner verificado: `bash scripts/gentle-ai.sh`;
- configuración de Codex: `.codex/config.toml`;
- skill opt-in de review: `.agents/skills/gentle-review/SKILL.md`;
- evaluación y rollback: `docs/experiments/gentle-ai-codex.md`.

No ejecutes `gentle-ai install` directamente sobre el repositorio ni sobre el HOME del usuario: la RC genera rutas no portables, duplica instrucciones y realiza escrituras globales incluso con `--scope workspace`.
