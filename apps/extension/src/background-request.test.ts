...
describe("buildApiRequest", () => {
  ...
  it("conserva valores numéricos cero en multipart", () => {
    const { options } = buildApiRequest(
      {
        type: "API_REQUEST",
        method: "POST",
        endpoint: "/entries/",
        body: { title: "Prólogo", progress_total: 0 },
        isFormData: true,
      },
      "http://localhost:8000",
      "device-token",
    ...
    const formData = options.body as FormData;
    expect(formData.get("title")).toBe("Prólogo");
    expect(formData.get("progress_total")).toBe("0");
  });
